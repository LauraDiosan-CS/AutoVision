...
class LaneDetect():
    ...
    def houghLines(self, masked_edges, rho=1, theta=np.pi / 180, threshold=50, min_line_length=100, max_line_gap=550):
        return cv2.HoughLinesP(masked_edges, rho, theta, threshold, np.array([]), minLineLength=min_line_length, maxLineGap=max_line_gap)
            ...
            for line in hough_lines:
                ...
                for x1, y1, x2, y2 in line:
                    ...
                    if not ((abs(y1 - y2) < 50) or (abs(x1 - x2) < 100)):
                        ...
                        if x2 < self.width /2 and abs(y1 - y2) == max_left_lane and not is_horizontal and not self.left_lane:
                            for i in range(2):
                                self.left_lane.append((coords[2 * i], coords[2 * i + 1]))

                        elif x2 > self.width /2 and abs(y1 - y2) == max_right_lane and not is_horizontal and not self.right_lane:
                            for i in range(2):
    ...
    def extend_lanes(self):
        ...
        if self.left_lane and self.right_lane:
            ...
            right_slope = (self.right_lane[1][1] - self.right_lane[0][1]) / (self.right_lane[1][0] - self.right_lane[0][0])
            ...
            if left_distance < right_distance:
                ...
                intersection_x_right = self.find_intersection_x_coordinate(right_slope, y_intercept_right, minimum_startpoint[1])
                ...
                intersection_x_left = self.find_intersection_x_coordinate(left_slope, y_intercept_left, minimum_startpoint[1])
    ...
    def steering_angle(self):
        if self.left_lane and self.right_lane:
            ...
            cv2.putText(self.frame, f'Steering angle: {steering_angle:.2f}', (100, 300), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 255, 255), 6)
    ...
    def define_lane_area(self):
        if self.left_lane and self.right_lane:
            roi_points = np.array([self.left_lane[0], self.left_lane[1], self.right_lane[1], self.right_lane[0]], dtype=np.int32)
