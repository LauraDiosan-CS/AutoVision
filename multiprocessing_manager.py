    ...
    def run(self):
        ...
        while self.keep_running.value:
            for reader in shared_memory_readers:
                ...
                pipe_data.timing_info.stop("Transfer Data (SPF -> MM)")

                if current_pipe_data:
                    current_pipe_data.merge(pipe_data)
                    # print("merging")
                else:
                    current_pipe_data = pipe_data

                pipe_data.timing_info.start("Transfer Data (MM -> Viz+)", parent=f"Data Lifecycle {pipe_data.last_touched_process}")
